...
def get_same_padding_conv2d(image_size=None):
    ...
    if image_size is None:
        ...
        # image_size作为参数传入Conv2dStaticSamePadding的形参image_size中，并形成一个新的函数
...
class BlockDecoder(object):
    ...
    def _decode_block_string(block_string):
        ...
        options = {}
        for op in ops:
            splits = re.split(r'(\d.*)', op)  # 阿拉伯数字+任意字符(除换行符)(匹配0个或多个)
            if len(splits) >= 2:
                key, value = splits[:2]
                options[key] = value
...
def efficientnet(width_coefficient=None, depth_coefficient=None, dropout_rate=0.2,
    ...
    blocks_args = BlockDecoder.decode(blocks_args)  # len(blocks_args) is 7
...
def get_model_params(model_name, override_params):
    ...
    return:
    blocks_args: list len(blocks_args) == 7
    global_params:
    """
    if model_name.startswith('efficientnet'):
        # efficientnet的一些参数信息
        w, d, s, p = efficientnet_params(model_name)  # width_multi、depth_multi、input_h、dropout_rate
    ...
    if override_params:
        ...
        global_params = global_params._replace(**override_params)  # 替换override_params这个关键字段的信息
    return blocks_args, global_params
...
def load_pretrained_weights(model, model_name, load_fc=True, advprop=False):
    """ Loads pretrained pretrain_weights, and downloads if loading for the first time. """
    # AutoAugment or Advprop (different preprocessing)
    url_map_ = url_map_advprop if advprop else url_map
    state_dict = model_zoo.load_url(url_map_[model_name], map_location=torch.device('cpu'))
    # state_dict = torch.load('../../pretrain_weights/backbone_efficientnetb0.pth')
    if load_fc:
        ...
        assert set(res.missing_keys) == set(['_fc.weight', '_fc.bias']), 'issue loading pretrained pretrain_weights'
    print('Loaded pretrained pretrain_weights for {}'.format(model_name))
